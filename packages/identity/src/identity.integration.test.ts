...
describe("IdentityService — split provider ID collapse", () => {
  it("maps ESPN 229 and FIFA bra to one canonical team", async () => {
    const canonicalId = "bra";
    const repo = mockRepo({
      findAlias: vi.fn(async (providerId, externalId, entityType) => {
        if (entityType !== "team") return null;
        if (providerId === "espn" && externalId === "229") {
          return {
            id: "a-espn",
            canonicalId,
            entityType: "team",
        ...
        if (providerId === "fifa" && externalId === "bra") {
          return {
            id: "a-fifa",
            canonicalId,
            entityType: "team",
    ...
    const espn = await svc.resolveToCanonical("espn", "229", "team", "Brazil");
    const fifa = await svc.resolveToCanonical("fifa", "bra", "team", "Brazil");

    expect(espn.resolved).toBe(true);
    expect(fifa.resolved).toBe(true);
    if (espn.resolved && fifa.resolved) {
      expect(espn.canonicalId).toBe(fifa.canonicalId);
      expect(espn.canonicalId).toBe("bra");
    }
  });
});
